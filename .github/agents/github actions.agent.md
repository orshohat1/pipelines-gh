...
tools: [execute, read/readFile, edit/editFiles, search, web]
---
...
| `task: UsePythonVersion@0` | `actions/setup-python@v5` |
...
| Variable groups | Environment secrets / vars |
| Template references (`template:`) | Reusable workflows (`uses:`) or composite actions |
...
| `image:` | `container: image:` or `runs-on:` |
| `script:` | `- run:` steps |
...
| `only: / except:` | `on:` triggers + `if:` conditions |
| `rules:` | `on:` triggers + job `if:` |
| `artifacts:` | `actions/upload-artifact@v4` |
| `cache:` | `actions/cache@v4` |
| `services:` | `services:` in job container config |
| `include:` (templates) | Reusable workflows (`uses:`) |
...
| `retry:` | Third-party retry actions or step-level retry |
